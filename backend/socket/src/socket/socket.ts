...
export default function registerChatSocket(io: Server) {
  io.on("connection", (socket: Socket) => {
    ...
    socket.on("join-room", async ({ username, room }) => {
      ...
      io.to(room).emit("online-users", Array.from(onlineUsers.values()));
    ...
    socket.on("message-delivered",async ({ messageId, room }) => {
        await Message.findByIdAndUpdate( messageId,{status:"delivered" });
        io.to(room).emit( "message-delivered",{messageId });
      });
      
      
      socket.on("message-seen", async ({messageId, room})=>{
...
//       io.to(data.room).emit("message-delivered", {messageId: msg._id});
//     });
